      ...
      <div className="container mx-auto px-4 max-w-6xl">
        <div className="text-center mb-10 reveal">
          ...
          <p className="text-gray-600 text-lg md:text-xl font-medium leading-relaxed max-w-3xl mx-auto">
            Na RER Drywall e Pintura, unimos a precisão técnica do Drywall com o acabamento refinado da pintura profissional.
          </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
          {authorityPoints.map((point, index) => (
            <div key={index} className={`reveal delay-${index * 100} bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-xl hover:shadow-2xl transition-all duration-500 group flex flex-col items-center text-center hover:-translate-y-3`}>
              <div className="bg-blue-50 w-20 h-20 rounded-3xl flex items-center justify-center mb-8 group-hover:bg-red-600 transition-colors duration-500">
                <div className="group-hover:text-white transition-colors duration-500">
                  {point.icon}
                </div>
              </div>
              <h3 className="text-xl font-extrabold text-blue-900 mb-4 group-hover:text-red-600 transition-colors">{point.title}</h3>
              <p className="text-gray-500 font-semibold leading-relaxed text-sm">{point.desc}</p>
            </div>
          ))}
        </div>

        <div className="reveal bg-white rounded-[3rem] p-8 md:p-10 shadow-2xl border border-gray-100 flex flex-col items-center">
          <div className="flex flex-col sm:flex-row flex-wrap justify-center gap-6 md:gap-12 mb-8">
          ...
          <a href={COMPANY.whatsappUrl} className="inline-flex items-center bg-red-600 text-white px-8 py-4 rounded-2xl font-black text-base hover:bg-blue-900 transition-all shadow-xl hover:scale-105">
